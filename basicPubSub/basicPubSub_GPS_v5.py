...
def CameraVideo(cdtime):
	...
	#os.system("avconv -f video4linux2 -r 10 -s 1280x960 -i /dev/video0 /home/pi/aws-iot-device-sdk-python/samples/basicPubSub/Camera/Rpi_video_TEST.avi")
...
while True:
    ...
    if args.mode == 'both' or args.mode == 'publish':
		...
		# GeoFence
		#message['geofence'] = GeoFence(message['location'], zone)
		# GeoFence
                #message['geofence'] = GeoFence(message['location'], zone)
                message['SM_geo'] = GeoFence(message['location'], zone1)
                message['CV_geo'] = GeoFence(message['location'], zone2)
		...
		'''
                ...
                if len(gpsList) == 10:
                        try:
                                print('Dumping')
                                with open("GPS_DATA_{}.txt".format(tripstart), mode='aw') as f:
                                        json.dump(gpsList, f)
					json.dump('\n')
